...
#Armazena indereço de scena da bala
var pre_bullet = preload("res://Scenes/bullet.tscn")

export var loaded = true
...
func _physics_process(delta):
	...
	#Faz o canhão olhar para o mouse
	...
	if Input.is_action_pressed("shoot") and loaded:
		_TiroDoJogador()
...
func _TiroDoJogador():
	...
	bullet.global_position = $cannon/muzzle.global_position
	bullet.direcao = Vector2(cos($cannon.global_rotation) , sin($cannon.global_rotation)).normalized()
	$"../".add_child(bullet)
	$CadenciaDeTiro.wait_time = bullet.cadencia
	loaded = false
	$CadenciaDeTiro.start()
